...
STATUS="$(ssh "${REMOTE_HOST}" "PROJECT='${REMOTE_PROJECT_DIR}' bash -s" <<'EOF'
set -euo pipefail
cd "${PROJECT}"
...
cache_workers="$({ pgrep -af 'cache_wan_latents.py .*cache_wan_latents_720_4k_1000.yaml' 2>/dev/null || true; } | wc -l)"
train_workers="$({ pgrep -af 'scripts/train_lsr.py --config configs/train_lsr_720_4k_1000.yaml' 2>/dev/null || true; } | wc -l)"
torchrun_workers="$({ pgrep -af 'torch.distributed.run .*train_lsr_720_4k_1000.yaml' 2>/dev/null || true; } | wc -l)"
step="0"
...
elif (( hr_count < 1000 && cache_workers == 0 )); then
...
if [[ -z "${repair_reason}" && "${parent_alive}" == "1" && "${hr_count}" -lt 1000 && "${cache_workers}" -lt 10 ]]; then
  ...
  if (( count >= 2 )); then
    repair_reason="cache worker count stayed below 10 for ${count} checks"
  else
    echo "repair_deferred=cache worker count ${cache_workers}/10; will repair if repeated"
  fi
else
  rm -f "${state_file}"
fi
