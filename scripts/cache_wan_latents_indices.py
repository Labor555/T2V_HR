...
def parse_args():
    ...
    parser.add_argument("--indices", required=True, help="Comma-separated global indices or inclusive ranges.")
...
def main() -> None:
    ...
    if limit and limit > 0:
        rows = rows[:limit]
    requested = [idx for idx in _parse_indices(args.indices) if 0 <= idx < len(rows)]
    if not requested:
        raise ValueError("No valid indices to cache")
    ...
    index_rows = []
    for idx in tqdm(requested, desc=f"cache_wan_latents_indices[{args.index_name}]"):
        row = rows[idx]
